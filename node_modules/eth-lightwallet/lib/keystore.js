...
var ethtxjs = require('ethereumjs-tx');
var Transaction = ethtxjs.transaction;
var SwhTransaction = ethtxjs.swhtransaction;
var TxInput = ethtxjs.txinput;
var TxOutput = ethtxjs.txoutput;
var rlp = require('rlp');
...
KeyStore.prototype.signTransaction = function (txParams,chainType, callback) {
  var _this = this
  ...
  if(chainType == "swh" &&( txParams.inputs||txParams.outputs)){
    //ethjsTxParams.v = 0;
    txObj = new Transaction(ethjsTxParams);
    var inputs = txParams.inputs;
    var outputs = txParams.outputs;
    ...
    var outputsarr = [];
    for(inp in inputs){
      inputsarr[inp] = add0x(new TxInput(inputs[inp]).serialize().toString('hex'));
    }
    for(outp in outputs){
      outputsarr[outp] = add0x(new TxOutput(outputs[outp]).serialize().toString('hex'));
    }
    //dataIn.Vin = inputsarr;
    //dataIn.Vout = outputsarr;
    dataIn.Vin = rlp.encode(inputsarr);
    dataIn.Vout = rlp.encode(outputsarr);

    var date = new Date();
    udate = Date.UTC(date.getFullYear(),date.getMonth(),date.getDate(),date.getHours(),date.getMinutes(), date.getSeconds());
    dataIn.Timestamp = (udate/1000);

    dataIn.Data = add0x(txObj.serialize().toString('hex'));
    //dataIn.Data = txObj;
    txObj = new SwhTransaction(dataIn);
  }
  ...
  this.passwordProvider( function (err, password, salt) {
    ...
    _this.keyFromPassword(password, function (err, pwDerivedKey) {
      if (err) return callback(err);
      var signedTx = signing.signTx(self, pwDerivedKey, rawTx, signingAddress, self.defaultHdPathString, self.chainType);
